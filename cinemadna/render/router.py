...
MODEL_REGISTRY: Final[dict[str, dict[str, Any]]] = {
    "local-acting-v1": {
        "tier": "local",
        "good_at": (SHOT_CLOSEUP, "dialogue", "acting"),
        "cost_per_sec": 0.05,
        "max_duration_sec": 30.0,
        "note": "本地微表情引擎，用于对白与表演",
    },
    "cloud-face-v1": {
        "tier": "cloud",
...
#: 本地特写/对白镜头拦截
LOCAL_ACTING_TYPES: Final[frozenset[str]] = frozenset({"dialogue", "acting"})
...
def route_shot(
    ...
    # 支持自定义传入的 scene_type 属性
    scene_type = contract.get("scene_type", stype)
    duration = float(contract["duration_sec"])
    hard = contract.get("difficulty") == "hard"

    # 拦截逻辑：如果是对白或情绪戏，直接发送给本地微表情引擎
    if scene_type in LOCAL_ACTING_TYPES or stype in LOCAL_ACTING_TYPES:
        model, reason = "local-acting-v1", "特写对白/情绪戏，切断云端请求，使用本地微表情引擎"
    elif stype in FACE_CRITICAL_TYPES:
